...
public class RequestCacheValue implements KVResponse {
    ...
    private RequestCacheValue(Builder builder) {
        ...
        switch (type) {
            ...
            case INTERNAL_ERROR:    this.errCode = KVServerTaskHandler.RES_CODE_INTERNAL_ER;        break;
            case OBITUARIES:
            {
    ...
    public DatagramPacket generatePacket() {
        //prepare checksum
        byte[] pl = this.generatePayload();
        long crc32 = getCRC(reqID, pl);

        byte[] fullMsg;
        //prepare message
        fullMsg = KVMsgSerializer.serialize(new UnwrappedMessage(reqID, pl, crc32));
